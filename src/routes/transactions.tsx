...
function Transactions() {
	...
	const updateTransactions = async (id: number, account_id: number, category_id: number, description: string, transaction_date: string, amount: number): Promise<void> => {
		try {
			await invoke("update_transaction", { id: id, accountId: account_id, categoryId: category_id, description: description, transactionDate: transaction_date, amount: amount });
									...
									<TableRow key={transaction.id} className="mb-2">
										<TableCell>
											<Input
												...
												onBlur={(e) => {
													updateTransactions(transaction.id, transaction.account_id, transaction.category_id, e.target.value, transaction.transaction_date, transaction.amount);
												}}
												className="w-36"
											/>
										</TableCell>
										<TableCell>
											<Select
												onValueChange={(e) => {
													updateTransactions(transaction.id, transaction.account_id, Number.parseInt(e), transaction.description, transaction.transaction_date, transaction.amount)
												}}
										...
										<TableCell>
											<Select
												onValueChange={(e) => {
													updateTransactions(transaction.id, Number.parseInt(e), transaction.category_id, transaction.description, transaction.transaction_date, transaction.amount)
												}}
											...
											<Popover>
												...
												<PopoverContent className="w-auto p-0" align="start">
													<Calendar
														...
														onSelect={(e) => {
															// @ts-ignore
															updateTransactions(transaction.id, transaction.account_id, transaction.category_id, transaction.description, e.toISOString(), transaction.amount);
														}}
														disabled={(date) =>
															date > new Date()
														}
										...
										<TableCell>
											<Input
												...
												onBlur={(e) => {
													...
													updateTransactions(transaction.id, transaction.account_id, transaction.category_id, transaction.description, transaction.transaction_date, centsValue);
												}}
												className="w-36"
											/>
										</TableCell>
