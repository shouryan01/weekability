...
export function TransactionForm({
	getCategories,
	setOpen,
}: { getCategories: () => Promise<void>; setOpen: (open: boolean) => void }) {
	...
	const getAccountNames = async (): Promise<void> => {
		try {
			...
			const account_names = raw_account_names.map(
	...
	async function onSubmit(transaction: z.infer<typeof FormSchema>) {
		try {
			await invoke("create_transaction", {
				...
				accountId: Number.parseInt(transaction.account_id),
				categoryId: Number.parseInt(transaction.category_id),
		...
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger>
				<Button
					variant="outline"
					size="icon"
					className="h-10 w-10 fixed bottom-0 right-0 transition duration-100 m-4"
				>
